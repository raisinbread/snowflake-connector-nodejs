...
function auth_web(connectionConfig, httpClient, webbrowser) {
  ...
  const cacheDirectory = './.snowflake_sso';
  if (!fs.existsSync(cacheDirectory)) {
    fs.mkdirSync(cacheDirectory);
  }
  const oneHourMs = 1000 * 60 * 60;
  const cacheExpiresInMs = oneHourMs; 
  ...
  this.authenticate = async function (authenticator, serviceName, account, username) {
    cacheFile =  cacheDirectory + `/${account}.${username}.json`;
    if (fs.existsSync(cacheFile)) {
      const cacheFileContents = fs.readFileSync(cacheFile);
      const cacheData = JSON.parse(cacheFileContents);

      if (cacheData.timestamp && Date.now() - cacheData.timestamp < cacheExpiresInMs) {
        token = cacheData.token;
        proofKey = cacheData.proofKey;
      } else {
        fs.rmSync(cacheFile);
      }
    }

    if (token !== undefined && proofKey !== undefined) {
      return;
    }
  ...
  function processGet(data)
  {
    ...
    const cacheData = { token, proofKey, timestamp: Date.now() };
