...
function auth_web(connectionConfig, httpClient, webbrowser) {
  ...
  let proofKey = process.env.SNOWFLAKE_PROOF_KEY ?? undefined;
  let token = process.env.SNOWFLAKE_TOKEN ?? undefined;
  ...
  this.authenticate = async function (authenticator, serviceName, account, username) {
    if (process.env.SNOWFLAKE_PROOF_KEY !== undefined && process.env.SNOWFLAKE_TOKEN !== undefined) {
      return;
    }
    ...
    process.env.SNOWFLAKE_PROOF_KEY = proofKey;
  ...
  function processGet(data)
  {
    ...
    process.env.SNOWFLAKE_TOKEN = token;
  };
